...
(function () {
  ...
  function renderDemoPanel() {
    ...
    // Demo mode is only used when somebody deliberately broadens @match for visual testing.
    // It never sends HERO, Rodeo, FCResearch, or employee lookups from a non-HERO page.
    _fcname = 'DEMO';
    _CustomerShipment = 'DEMO-SHIPMENT-001';
    panel.classList.add('is-demo');
    panel.classList.remove('is-loading');
    setTopPanelShipmentText();

    panel.querySelector('.heroLastRefresh').textContent = 'DEMO MODE';
    $('.ASIN').text('B0DEMOASIN');
    $('.pickTote').text('tsXDEMO001');
    $('.PickEventName').text('PICK_COMPLETE');
    $('.PickEventTime').text('Sample event');
    $('.packAA').text('demo.associate');
    $('.packStation').text('PACK-STATION-DEMO');
    $('.boxRec').text('S2');
    $('.Spoo').text('sp00DEMO');
    $('.PackEventName').text('COMPLETE_PACKAGE');
    $('.packEventTime').text('Sample event');
    $('.slamStation').text('SLAM-DEMO');
    $('.KOEventName').text('GET_SHIPPING_LABEL_FAILURE');
    $('.KOEventTime').text('Sample event');
    $('.KickoutCode').text('DEMO_ONLY');
    $('.AtropsIssue').text('DEMO').removeClass('is-alert');
    $('.AgeVerification').text('DEMO').removeClass('is-alert');
    $('.BridgingTT').text('Demo only - no ticket lookup sent');
    $('.associate').text('demo.associate');
    $('.reason').text('Sample defect data');
    $('.time').text('Sample event');
    $('.autoDwelled').text('NO').removeClass('is-yes');
    $('.manualDwelled').text('NO').removeClass('is-yes');
    $('.SRArea').text('P3');
    $('.SRTime').text('Sample event');
    $('.CategoryAccepted').text('DEMO');
    $('.ProcessPathChosen').text('PPPickToRebin3');
    $('.CatEventTime').text('Sample event');
    setFailureReason('Demo mode: this is sample display data only. No HERO requests were made.');
    setStage('pick', 'done', 'PICKED');
    setStage('pack', 'done', 'PACKED');
    setStage('slam', 'alert', 'DEMO');
    setStage('dwell', 'done', 'CLEAR');
    setStage('ps', 'idle', 'READY');

    const timelineButton = panel.querySelector('.heroTimelineToggle');
    if (timelineButton) {
      timelineButton.disabled = true;
      timelineButton.textContent = 'DEMO ONLY';
      timelineButton.title = 'Timeline needs a real HERO shipment page';
    }
  }
  ...
  function injectPanel() {
    ...
    document.getElementById('hero-tooltips-top-panel')?.classList.remove('is-demo');
    const timelineButton = document.querySelector('.heroTimelineToggle');
    if (timelineButton) {
      timelineButton.disabled = false;
      timelineButton.textContent = '⌁ TIMELINE';
      timelineButton.title = 'Open Shipment Timeline';
    }
