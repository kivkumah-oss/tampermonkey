...
(function () {
  ...
  function next() {
    return clickButton('Next', ['next', 'skip forward', 'forward']);
  }

  function previous() {
    return clickButton('Previous', ['previous', 'prev', 'skip back', 'back']);
  }

  function shuffle() {
    return clickButton('Shuffle', ['shuffle']);
  }

  function lyrics() {
    return clickButton('Lyrics', ['lyrics', 'show lyrics']);
  }

  function findTitle() {
    const candidates = Array.from(document.querySelectorAll('[data-testid],h1,h2,h3,a,span,div'))
      .map((el) => safeText(el.textContent))
      .filter((text) => text && text.length > 2 && text.length < 90)
      .filter((text) => !/play|pause|next|previous|shuffle|create|library|home|search/i.test(text));
  ...
  function styles() {
    return `
      ...
      #${PANEL_ID} .nsp-body{padding:10px;}
      #${PANEL_ID} .nsp-title{font-weight:700;color:#e5e7eb;line-height:1.3;min-height:32px;margin-bottom:8px;}
      #${PANEL_ID} .nsp-status{color:#9ca3af;font-size:11px;margin-top:8px;line-height:1.35;}
      #${PANEL_ID} .nsp-row{display:flex;gap:6px;flex-wrap:wrap;}
      ...
      #${PANEL_ID} .nsp-progress{height:5px;background:rgba(255,255,255,.12);border-radius:999px;overflow:hidden;margin-top:8px;}
      #${PANEL_ID} .nsp-progress > div{height:100%;width:0%;background:linear-gradient(90deg,#22d3ee,#a855f7);}
      #${PANEL_ID}.min .nsp-body{display:none;}
    `;
  }

  function injectStyles() {
    if (document.getElementById('nova-suno-player-style')) return;
    const style = document.createElement('style');
    style.id = 'nova-suno-player-style';
  ...
  function createPanel() {
    ...
    panel.innerHTML = `
      <div class="nsp-head">
        ...
        <span>
          <button class="nsp-small" data-nsp-refresh>↻</button>
          <button class="nsp-small" data-nsp-min>−</button>
        </span>
      </div>
      <div class="nsp-body">
        <div class="nsp-title" data-nsp-title>Ready</div>
        <div class="nsp-row">
          <button data-nsp-prev>⏮</button>
          <button data-nsp-play>▶ / ⏸</button>
          <button data-nsp-next>⏭</button>
          <button data-nsp-shuffle>🔀</button>
          <button data-nsp-lyrics>Lyrics</button>
        </div>
        <div class="nsp-progress"><div data-nsp-bar></div></div>
        <div class="nsp-status" data-nsp-status>Waiting for Suno audio...</div>
      </div>
    `;
    ...
    panel.querySelector('[data-nsp-min]').addEventListener('click', () => {
  ...
  function refresh() {
    ...
    if (audio) {
      ...
      setStatus('No audio element found yet. Start a Suno track first.');
    }
  }
